...
EXCLUDE_ONTOLOGY={
  7:"A conversa é a modalidade de crítica de interfaces; o objeto é uma ferramenta de apoio ao design visual, não um produto conversacional.",
...
FULL_TEXT={
  ...
  45:"A integração entre recomendação e CUI pode ser predominantemente arquitetural; o texto integral deve demonstrar implicações para produto e interação.",
  ...
  67:"O título sugere implementação em plataforma empresarial, mas é preciso verificar se há contribuição de interação e produto além da automação técnica.",
  77:"O título é aderente, porém a ausência de resumo impede avaliar método, contribuição e alcance das heurísticas."
}

FIELDS_EXTRA=["manual_centrality","manual_product_relevance","manual_documentary_evidence","manual_actionable_contribution","manual_total","screening_decision","screening_reason","evidence_basis","reviewer","review_date","author_review"]
...
def main():
  ...
  for index,row in enumerate(rows,1):
    ...
    if index in EXCLUDE_ONTOLOGY:
      decision="exclude-ontology"; reason=EXCLUDE_ONTOLOGY[index]; centrality=2; product=1; actionable=0; author_review="sample-audit"
    elif index in EXCLUDE_DUPLICATE:
      decision="exclude-duplicate"; reason=EXCLUDE_DUPLICATE[index]; centrality=4; product=3; actionable=1; author_review="not-required"
    elif index in FULL_TEXT:
      decision="full-text-review"; reason=FULL_TEXT[index]; centrality=3; product=2; actionable=0; author_review="required"
    else:
      decision="provisional-include"; centrality=4 if int(row.get("ontology_score") or 0)==4 else 3
      product=3 if any(x in ((row.get("title") or "").lower()) for x in ["design","guideline","principle","framework","pattern","usability","accessible","error","ethic","evaluation"]) else 2
      actionable=1
      reason="Título e resumo tratam o design, a avaliação ou as consequências de uma interface, agente ou sistema conversacional como contribuição central. A decisão será confirmada no documento."
      author_review="required-if-central-claim"
    documentary=2 if has_pdf else 1
    total=centrality+product+documentary+actionable
    row.update({"manual_centrality":centrality,"manual_product_relevance":product,"manual_documentary_evidence":documentary,
