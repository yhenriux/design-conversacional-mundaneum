...
def fetch(item, timeout):
 ...
 try:
  ...
  with urllib.request.urlopen(req,timeout=timeout) as response:data=response.read();out["content_type"]=response.headers.get("Content-Type","")
...
def main():
 import argparse
 parser=argparse.ArgumentParser(); parser.add_argument("--timeout",type=int,default=15); parser.add_argument("--workers",type=int,default=8); args=parser.parse_args()
 selected=[];seen=set()
 ...
 with ThreadPoolExecutor(max_workers=args.workers) as pool:
  for future in as_completed([pool.submit(fetch,x,args.timeout) for x in selected]):result.append(future.result())
