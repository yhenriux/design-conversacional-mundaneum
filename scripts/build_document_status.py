...
def main():
 resolution=read("document-resolution.csv"); downloads={r["doi"]:r for r in read("download-manifest.csv")}; inspections={r["doi"]:r for r in read("pdf-inspection.csv")}
 rows=[]
 for r in resolution:
  d=downloads.get(r["doi"],{}); i=inspections.get(r["doi"],{})
  if i.get("correspondence")=="true": status="pdf-integral-validado-local"
  ...
  rows.append({"title":r["title"],"doi":r["doi"],"screening_decision":r["screening_decision"],"document_status":status,"is_oa":r["is_oa"],
   "pdf_url":r["pdf_url"],"license":r["license"],"local_path":d.get("path","") if i.get("correspondence")=="true" else "","pages":i.get("pages",""),
   "sha256":d.get("sha256","") if i.get("correspondence")=="true" else "","retrieval_error":d.get("error","")})
 fields=list(rows[0]);
